...
- 名前だけ(例: `おくすり`)を送ると、時間と曜日をボタンで選んで登録
  - 時間はLINE純正の時刻ピッカー、曜日は `毎日` / `平日` / `週末` / 各曜日のボタンから選択
- `火曜日 電車 8:00` のように曜日を付けると、その曜日だけお知らせ
  - `火曜日` / `毎週火曜` → 毎週火曜だけ
  - `月水金` / `月・水・金` → 月・水・金だけ
  - `平日` → 月〜金、`土日` / `週末` → 土・日
  - 曜日を付けない(または `毎日`)と毎日お知らせ
- `一覧` と送ると本人のLINEだけに登録内容を表示
- 一覧から本人がリマインダーを削除
- 一覧の `時間を変える` から通知時刻を変更
- 一覧の `曜日を変える` から通知曜日を変更(曜日ボタン、または `月水金` のように送信)
- 指定時刻にLINEへPush通知
- 通知の `あとで` を押すと、15分後・30分後・1時間後・好きな時間・通知しない、から選べる
...
1. 祐希ちゃんがLINEで `重曹クエン酸水 8:00` のように送る
   - 曜日を決めたいときは `火曜日 電車 8:00`(毎週火曜だけ)や `平日 おくすり 21:00`(月〜金だけ)のように送る
2. 確認カードの `登録する` を押す
   - 内容を直したいときは `編集する` を押すと、時間・曜日の変更や取り消しが選べる
3. 時間になるとLINEで `祐希ちゃん、そろそろだよ` と届く
4. `飲んだよ` または `やったよ` を押す
5. `いいこ、いいこ、よくできました。` が返る

まだ飲めないときは、通知カードの `あとで` を押して、15分後・30分後・1時間後・好きな時間、から選ぶと、その時間にもう一度お知らせします。今日は無理なときは `通知しない` を選べます。
...
- 一覧カードの `曜日を変える` を押すと曜日ボタンが出る。ボタンを押すか、`月水金` のように送ると曜日を変更できる
- 時間や曜日の変更は10分以内に答えなかった場合は取り消され、ふつうのメッセージとして扱われる
...
既存DBへ「名前だけ送って、時間と曜日をボタンで選ぶ登録」用の表を追加する場合は、Supabase SQL Editorで以下を実行します。

```text
supabase/20260712_add_pending_registrations.sql
```

既存DBで一覧の `曜日を変える` を使えるようにする場合は、Supabase SQL Editorで以下を実行します。

```text
supabase/20260712_add_pending_edit_kind.sql
```
